...
rm(list=ls())
libraries_check <- c("data.table","igraph","survival","stats","msm")
...
nsimobs <- 20 # number of simulated datasets

# all possible simulation settings
sim_settings <- expand.grid(model_sim=c("add"),
                            network_sim=c("LH","PA"),
                            m_sim=c(124,120,96,64,32),
                            failrate_sim=0:1)
sim_settings <- sim_settings[rep(1:nrow(sim_settings),each=200),]
...
mu_true <- 4.5
sigma_true <- .25
y_unif <- exp(rnorm(n,mean=mu_true,sd=sigma_true))
...
for (sim_obs in 1:nsimobs) {
  ...
  # # Z=1
  cens_Z1_drop <- exp(rnorm(n,mean=mu_true+tau_true*G,sd=sqrt(1-sigma_true^2)))
...
results[,lapply(.SD, length),by=list(model,network,failrate,m),.SDcols="sim"]
